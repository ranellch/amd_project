...
%WAVEFAST Perform multi-level 2D fast wavelet transform
%   [C, L] = WAVEFAST(X, N, LP, HP) performs a 2D N-level FWT of image (or
%   matrix) X with respect to decomposition filters LP and HP.
%
%   [C, L] = WAVEFAST(X, N, WNAME) performs the same operation but fetches
%   filters LP and HP for wavelet WNAME using WAVEFILTER
%
%   Scale parameter N must be less than or equal to log2 of the maximum
%   image dimension.  Filters LP and HP must be even.  To reduce border
%   distortion, X is symmetrically extended.  I.e. if X = [c1 c2 c3...cn]
%   in 1D then its symmetric extension would be [... c3 c2 c1 c1 c2 c3 ...
%   cn cn-1 cn-2 ...].
%
%   OUTPUTS:
...
%       matrices, respectively.  C has 3n+1 sections where n is the number
%       of wavelet deocmpositions.
%
...
%
%   Related functions: WAVEBACK, WAVEFILTER.
...
if ~ismatrix(x) || (min(sx) < 2) || ~isreal(x) || ~isnumeric(x)
...
%Initializae the starting output data structures and initial approximation
c = []; s = sx; app = double(x);
...
for i = 1:n
    ...
    %and LP to get the horizontal and next approximation coefficients.
...
%-------------------------------------------------------------------------
    function  [y,keep] = symextend(x, fl)
        ...
        %downsampling.  Then extend x in both dimensions.
...
%-------------------------------------------------------------------------
    function y = symconv(x, h, type, fl, keep)
        %Convolve  the rows or columns of x with h, downsample, and extract
        %the center section since symmetrically extended.
