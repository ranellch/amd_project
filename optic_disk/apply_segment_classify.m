function [bin_image, prob_matrix] = apply_segment_classify(subimg, number_of_pixels, text_prediction_struct, int_prediction_struct)
    %Get the number of boxes
    iterations = floor(size(subimg, 1) / number_of_pixels);
    ...
    for x=1:iterations
        for y=1:iterations
            ...
            if(xs + number_of_pixels > size(subimg, 2))
                xs = size(subimg, 2)-number_of_pixels;
            end
            
            ys = ((y - 1) * number_of_pixels) + 1;
            if(ys + number_of_pixels > size(subimg, 1))
                ys = size(subimg, 1)-number_of_pixels;
            end
            
            %Get the subimage grid for each part of the subimage
            subimage = subimg(ys:ys+number_of_pixels,xs:xs+number_of_pixels);
